 ...
 */
static void get_row_png (Parameters params)
{
   int i;
...
static void start_input_png (Parameters params)
{
   ...
   /* ensure grayscale pixels use 4 bytes */
   ...
   if (source->row_pointers) {
      ...
      while((!fail && (row < height))) {
         ...
         source->row_pointers[row] = (png_bytep) malloc(width * 4);
...
Parameters png_init ()
{
   ...
   if (source != NULL) {
      ...
      source->pub.get_pixel_row = get_row_png;
