   ...
   implements ImageProducer
{
    ...
    */
   public void setImageRow(int row, int[] rowData)
   {
      // ensure row is valid
      if ((row < 0) || (row > height))
   ...
   public void startProduction(ImageConsumer ic)
   {
      ...
      for(i=0; i<count; i++)
         consumers[i].setColorModel(model);
      ...
      for(i=0; i<numChunks; i++)
      {
         for(int j=0; j<count; j++)
            consumers[j].setPixels(0, thisRow, width, rowsForChunk[i], model,
                                   data[i], 0, width);
   ...
   public void requestTopDownLeftRightResend(ImageConsumer ic)
   {
      ...
      {
         ic.setPixels(0, thisRow, width, rowsForChunk[i], model, data[i],
                      0, width);
         thisRow += rowsForChunk[i];
      }
